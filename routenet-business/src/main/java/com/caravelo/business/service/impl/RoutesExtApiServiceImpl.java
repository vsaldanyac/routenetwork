...
public class RoutesExtApiServiceImpl implements RoutesExtApiService {
	...
	public List<StationData> getRoutes() throws EndpointNotAvailableException, ParsingExternalDataException {
		try {
			...
			UriComponentsBuilder builder = UriComponentsBuilder.fromUriString(apiService);
			ResponseEntity<StationData[]> response = restTemplate.exchange(builder.buildAndExpand(urlParams).toUri(), HttpMethod.GET, entity, StationData[].class);
